...
def main():
    ...
    parser.add_argument("--model", type=str, default=s.bge_model_name)  # BAAI/bge-m3
    ...
    for batch in tqdm(batched(rows, args.batch_size), desc="Ingest BGE", unit="batch"):
        ...
        for r, vec in zip(batch, vectors):
            ...
            points.append(
                models.PointStruct(
                    id=chunk_id_to_int(r["chunk_id"]),  # use chunk_id as point id
                    vector=vec.tolist(),
                    payload=payload,
                )
            )
